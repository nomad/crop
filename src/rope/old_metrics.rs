...
use super::gap_buffer::GapSummary;
...
impl Metric<RopeChunk> for ByteMetric {
    ...
    #[inline]
    fn measure(summary: &GapSummary) -> Self {
        Self(summary.bytes)
    }
}
...
impl SlicingMetric<RopeChunk> for ByteMetric {
    ...
    fn split<'a>(
        ...
        &summary: &GapSummary,
    ) -> (&'a ChunkSlice, GapSummary, &'a ChunkSlice, GapSummary)
    where
        'a: 'a,
    {
        if up_to == chunk.len() {
            (chunk, summary, "".into(), GapSummary::default())
        } else {
...
impl SlicingMetric<RopeChunk> for RawLineMetric {
        ...
        'a: 'a,
    {
        ...
        let left_summary = GapSummary { bytes: left_bytes, line_breaks: at };

        let right = chunk[left_bytes..].into();

        let right_summary = GapSummary {
...
impl UnitMetric<RopeChunk> for RawLineMetric {
    ...
    fn first_unit<'a>(
        chunk: &'a ChunkSlice,
        summary: &GapSummary,
    ) -> (&'a ChunkSlice, GapSummary, GapSummary, &'a ChunkSlice, GapSummary)
    where
        'a: 'a,
    {
...
impl DoubleEndedUnitMetric<RopeChunk> for RawLineMetric {
    ...
    fn last_unit<'a>(
        chunk: &'a ChunkSlice,
        summary: &GapSummary,
    ) -> (&'a ChunkSlice, GapSummary, &'a ChunkSlice, GapSummary, GapSummary)
    where
        'a: 'a,
    {
        let mut last_summary =
            GapSummary { bytes: summary.bytes, line_breaks: 0 };
